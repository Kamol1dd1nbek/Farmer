...
export class CowroomTypesService {
  ...
  async create(
    createCowroomTypeDto: CreateCowroomTypeDto,
  ): Promise<CowroomType> {
    let { name } = createCowroomTypeDto;
    name = name.toUpperCase();
    const cowroomType = await this.cowroomTypeModel.create({
      ...createCowroomTypeDto,
      name,
    });
    return cowroomType;
  }
  ...
  async update(
    id: string,
    updateCowroomTypeDto: UpdateCowroomTypeDto,
  ): Promise<CowroomType> {
    if (!isValidObjectId(id)) {
      throw new BadRequestException('Invalid Id');
    }

    const updatedCowroomType = await this.cowroomTypeModel.findOneAndUpdate(
      { _id: id },
      updateCowroomTypeDto,
      { new: true },
    );
  ...
  async remove(id: string) {
    ...
    const removedCowroomType = await this.cowroomTypeModel.findOneAndDelete({
      _id: id,
    });
