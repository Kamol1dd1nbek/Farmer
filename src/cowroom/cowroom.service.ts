...
export class CowroomService {
  ...
  async create(createCowroomDto: CreateCowroomDto): Promise<Cowroom> {
    ...
    const cowroom = await this.cowroomModel.create({
      ...createCowroomDto,
      name,
    });
    return cowroom;
  ...
  async update(
    id: string,
    updateCowroomDto: UpdateCowroomDto,
  ): Promise<Cowroom> {
    if (!isValidObjectId(id)) {
      throw new BadRequestException('Invalid Id');
    }

    const updatedCowroom = await this.cowroomModel.findOneAndUpdate(
      { _id: id },
      updateCowroomDto,
      { new: true },
    );
  ...
  async remove(id: string) {
    ...
    const removedCowroom = await this.cowroomModel.findOneAndDelete({
      _id: id,
    });
